...
public:
    int ans = 0;
    int curr_sum = 0;
    pair<bool, int> f(TreeNode *root, int low, int high)
    {
        if (root == NULL)
            return {true, 0};

        pair<bool, int> b1 = f(root->left, INT_MIN, root->val);

        pair<bool, int> b2 = f(root->right, root->val, INT_MAX);
        int sum =0;
        if (b1.first && b2.first)
        {
            sum += root->val + b1.second + b2.second;
            if (ans < sum)
                ans = sum;
            // cout << root->val << ' ' << low << ' ' << high <<  ' ' << curr_sum << endl;
